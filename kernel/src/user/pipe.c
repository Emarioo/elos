...
int write(Handle _handle, const void* buffer, uint64_t size) {
    ...
    while (remainingBytes) {
        int64_t suspected_freeSpace = handle->ring_buffer_size + handle->tail_commit - handle->head_reserve;
        ...
        while (old_head + bytesToWrite - handle->tail_commit > handle->ring_buffer_size) {
        ...
        while (handle->head_commit != old_head) {
            pause();
            if((handle->status & PIPE_STATUS_ENABLED) == 0) {
                // We leave ring buffer in unstable state.
                // head_reserve != head_commit
...
int read(Handle _handle, void* buffer, uint64_t size) {
    ...
    while (remainingBytes) {
        int64_t suspected_usedSpace = handle->head_commit - handle->tail_reserve;
        ...
        while (handle->head_commit - old_tail < bytesToRead) {
            // Other thread wrote and there's no space.
            // Wait until our reserved space can be written to.
            pause();
            if((handle->status & PIPE_STATUS_ENABLED) == 0) {
        ...
        while (handle->tail_commit != old_tail) {
            pause();
            if((handle->status & PIPE_STATUS_ENABLED) == 0) {
                // We leave ring buffer in unstable state.
                // head_reserve != head_commit
