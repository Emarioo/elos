...
void fat16_init_table(fat16* data) {
    char* entries = (uint8_t*)data + 512;
    int entry_count = (9 * 512 * 8) / 16; // 9 sectors * 512 bytes * 8 bits / 16 bits per entry

    // clear table
    for (int i=0;i<entry_count;i++) {
        fat16_write_table_entry(entries, i, FAT16_UNUSED_CLUSTER);
    }
    fat16_write_table_entry(entries, 0, 0xFFF0); // same value as BPB_media descriptor in boot.asm
    fat16_write_table_entry(entries, 1, 0xFFFF);

    int entries_size = 9 * 512;
    char* entries_mirror = entries + entries_size; // offset to second file allocation table
    memcpy(entries_mirror, entries, entries_size);
...
int fat16_create_entry(fat16* data, char* name, bool is_dir) {
    ...
    int cluster = fat16_alloc_cluster(data);
    if(cluster == -1)
        return 0;
    ...
    char* table = (uint8_t*)data + 512;
    fat16_write_table_entry(table, cluster, FAT16_LAST_CLUSTER_MIN);

    return found_entry;
}

void fat16_write_table_entry(char* entries, int entry_index, int value) {
    int off = entry_index * 2;
    *(uint16_t*)(entries + off) = value;
}
...
void fat16_write_file(fat16* data, int entry_index, int offset, char* buffer, const int size) {
    DirectoryEntry* root_entries = (DirectoryEntry*)((char*)data + 19 * 512); // root starts at sector index 19
    DirectoryEntry* entry = root_entries + entry_index;
    char* table = (uint8_t*)data + 512;
    ...
    while(remaining_size > 0) {
        if(offset < head + 512) {
            ...
            if(off < 0)
                off = 0;
            char* cluster_data = fat16_get_cluster_memory(data, cluster);
            
            int bytes = remaining_size - off;
            if (bytes > 512) {
                bytes = 512;
            }
        ...
        // next cluster
        int value = fat16_read_table_entry(table, cluster);
        if (value >= FAT16_LAST_CLUSTER_MIN && value <= FAT16_LAST_CLUSTER_MAX) {
            // alloc new cluster
            int new_cluster = fat16_alloc_cluster(data);
            fat16_write_table_entry(table, cluster, new_cluster); // old cluster points to new cluster
            fat16_write_table_entry(table, new_cluster, FAT16_LAST_CLUSTER_MIN); // new cluster is the end
            cluster = new_cluster;
            char* cluster_data = fat16_get_cluster_memory(data, new_cluster);
            memset(cluster_data, 0, 512);
        } else if (value > 0 && value < FAT16_RESERVED_CLUSTER_MIN) {
            // next cluster
            cluster = value;
        } else {
            // Assert(false); // bug
    ...
    int furthest_written_byte = offset + size;

    if (furthest_written_byte > entry->file_size)
        entry->file_size = furthest_written_byte;
}

int fat16_alloc_cluster(fat16* data) {
    char* entries = (uint8_t*)data + 512;
    int entry_count = 8 * 512 * 8 / 16; // 8 sectors * 512 bytes * 8 bits / 16 bits per entry

    int found_entry = -1;
    for (int i=0;i<entry_count;i++) {
        int value = fat16_read_table_entry(entries, i);
        if(value == FAT16_UNUSED_CLUSTER) {
            found_entry = i;
            break;
        } else if(value == FAT16_BAD_CLUSTER){
            break;
        }
    }
    if(found_entry == -1)
